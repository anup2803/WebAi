import { genereateResponse } from "../config/openRouter.js";
...
export const generateWebsite = async (req, res) => {
  try {
    ...
    if (!parased.code) {
      console.log("ai written invalid response", raw);
      return res.status(400).json({
        message: `ai returend invalid message`,
      });
    }
    ...
    console.log("error during prompting", error.response?.data || error);
...
export const changes = async (req, res) => {
  try {
    ...
    let parased = null;
    for (let i = 0; i < 2 && !parased; i++) {
      raw = await genereateResponse(updatePrompt);
      parased = await extractJson(raw);

      if (!parased) {
        raw = await genereateResponse(
          updatePrompt + "\n\nRETURN ONLY RAW JSON",
        );
        parased = await extractJson(raw);
      }
    ...
    website.conversation.push(
      ...
      { role: "ai", content: parased.message },
    );

    website.latestCode = parased.code;
    await website.save();
    user.credits = user.credits - 25;
    await user.save();
    return res.status(200).json({
      message: parased.message,
      code: parased.code,
      remainingCredits: user.credits,
    });
  } catch (error) {
    console.log("Update website error", error.response?.data || error);
  }
};
...
export const getAllWebsite = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "get all error website ", error });
  }
};

// deployed controller

export const delpoy = async (req, res) => {
  try {
    const website = await Website.findOne({
      _id: req.params.id,
      user: req.user._id,
    ...
    return res.status(500).json({ message: "deployed website error  ", error });
  }
};
